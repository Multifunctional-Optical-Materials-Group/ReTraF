...
function T = f_fit_RT_scatt(N, D,s00, lcoher, wl, theta, Re, Te, rscale, models,  x)
    ...
    for k1=1:length(theta)
        ...
        T = T + rscale^2*mean((Rt(:,k1)-Re(:,k1)-(1-alpha(k1))*Ae(:,k1)).^2);
        T = T + mean((Tt(:,k1)-Te(:,k1)-alpha(k1)*Ae(:,k1)).^2);
